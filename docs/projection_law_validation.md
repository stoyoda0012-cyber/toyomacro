...
inverse problems, derived in a companion theoretical manuscript (in preparation; reference
will be added when a preprint is available).*
...
effectively unregularized variable projection with $\sigma$ co-fit (numerical Tikhonov floor
$\lambda=10^{-8}$; $\lambda/\|\varphi\|^2 \approx 3\times10^{-9}$ in this configuration) —
matches the law's assumptions**
...
the law": the companion manuscript's depth-profiling instance uses an $\ell_1$-regularized
solver, a different estimator class, and there the ridge-linearized law is only
**directionally faithful (R² ≈ 0.91, n=8), with a systematic, uncalibrated gain ≈ 2.7×**.
...
| law (identifiable side) | slope 1.007, R² 0.999997 (pooled, n=288) | slope 0.9998, R² 1.0000000 |
| production solver | $\ell_1$ (sequential-quadratic): directional (gain ≈ 2.7×) | VarPro: exact (1.0004) |

*Regenerate:* `python examples/04_projection_law_validation.py` (numpy/scipy/matplotlib +
this package). Raw numbers are regenerated by the script into
`examples/output/04_projection_law_validation.json` (not tracked).
