...
This suite has **2,306 automated tests** across **86 files**, in two
locations:

| Location | Scope | Files | Tests |
|---|---|--:|--:|
| `tests/` | Library body — lineshapes, backgrounds, templates, I/O, quantification, meta | 48 | 1,256 |
...
- **Contract / regression** (1,874 tests, 81%) — guarantee the library
...
marker; `pytest -m "not perf"` drops them and leaves 2,294. Marking
...
## Library body — `tests/` (1,256)

### Claim guards — noise model, versions, backends, comparisons (78)
| Tests | File | Guards |
|--:|---|---|
...
| 7 | `test_benchmarks_doc_citations.py` | Every figure `docs/BENCHMARKS.md` attributes to a committed record is in that record, and an unattributed measurement says "no record" |
