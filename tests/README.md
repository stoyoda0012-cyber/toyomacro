...
This suite has **2,371 automated tests** across **86 files**, in two
locations:

| Location | Scope | Files | Tests |
|---|---|--:|--:|
| `tests/` | Library body — lineshapes, backgrounds, templates, I/O, quantification, meta | 48 | 1,301 |
| `src/toyomacro/voigtfit/tests/` | VoigtFit engine — solvers, encoders, information theory | 38 | 1,070 |
...
- **Contract / regression** (1,939 tests, 82%) — guarantee the library
...
marker; `pytest -m "not perf"` drops them and leaves 2,359. Marking
...
## VoigtFit engine — `src/toyomacro/voigtfit/tests/` (1,070)
...
### Infrastructure — compression, memory, I/O (178)
| Tests | File | Guards |
|--:|---|---|
| 36 | `test_memory.py` | Memory detection, optimal chunk size, dict3d cache size |
| 88 | `test_benchmark_record.py` | Cross-platform record schema: leaks no paths, names the backend, captures load, steal and thermal state |
