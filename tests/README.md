...
This suite has **2,368 automated tests** across **86 files**, in two
locations:

| Location | Scope | Files | Tests |
|---|---|--:|--:|
| `tests/` | Library body — lineshapes, backgrounds, templates, I/O, quantification, meta | 48 | 1,301 |
| `src/toyomacro/voigtfit/tests/` | VoigtFit engine — solvers, encoders, information theory | 38 | 1,067 |
...
- **Contract / regression** (1,936 tests, 82%) — guarantee the library
...
marker; `pytest -m "not perf"` drops them and leaves 2,356. Marking
...
| 52 | `test_benchmarks_doc_citations.py` | Every figure `docs/BENCHMARKS.md` attributes to a committed record is in that record, and an unattributed measurement says "no record" |
...
## VoigtFit engine — `src/toyomacro/voigtfit/tests/` (1,067)
...
### Infrastructure — compression, memory, I/O (175)
| Tests | File | Guards |
|--:|---|---|
| 36 | `test_memory.py` | Memory detection, optimal chunk size, dict3d cache size |
| 85 | `test_benchmark_record.py` | Cross-platform record schema: leaks no paths, names the backend, captures load, steal and thermal state |
