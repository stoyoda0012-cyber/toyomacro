...
This suite has **2,340 automated tests** across **86 files**, in two
locations:

| Location | Scope | Files | Tests |
|---|---|--:|--:|
| `tests/` | Library body — lineshapes, backgrounds, templates, I/O, quantification, meta | 48 | 1,281 |
...
- **Contract / regression** (1,908 tests, 81%) — guarantee the library
...
marker; `pytest -m "not perf"` drops them and leaves 2,328. Marking
...
## Library body — `tests/` (1,281)

### Claim guards — noise model, versions, backends, comparisons (103)
| Tests | File | Guards |
|--:|---|---|
...
| 32 | `test_benchmarks_doc_citations.py` | Every figure `docs/BENCHMARKS.md` attributes to a committed record is in that record, and an unattributed measurement says "no record" |
...
## VoigtFit engine — `src/toyomacro/voigtfit/tests/` (1,059)
...
### Infrastructure — compression, memory, I/O (167)
| Tests | File | Guards |
|--:|---|---|
| 36 | `test_memory.py` | Memory detection, optimal chunk size, dict3d cache size |
| 77 | `test_benchmark_record.py` | Cross-platform record schema: leaks no paths, names the backend, captures load, steal and thermal state |
