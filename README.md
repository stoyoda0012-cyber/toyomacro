...
| MLX / CUDA | **Experimental validation** — not a supported install target | manual setup; no extra ships for it | The MLX↔NumPy parity suite has passed twice on an RTX 5070 Laptop under WSL2, in 2026-07 and again 2026-09. Requires `MLX_ENABLE_TF32=0` (TF32 is on by default and silently costs precision) and multipeak batches ≤ 65,535 (upstream crash). No CUDA CI. Performance records for this configuration are committed under `benchmarks/records/`, and they are measurements of MLX's CUDA backend on one laptop GPU, not a supported performance claim: on that host plain NumPy beats CUDA on three of five solvers — the memory-bound ones are bound by PCIe under WSL2 (upstream `mlx#3861`), not by the card. See [`docs/CUDA_BACKEND_POC.md`](docs/CUDA_BACKEND_POC.md). |
