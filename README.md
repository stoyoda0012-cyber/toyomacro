...
| MLX / CUDA | **Experimental validation** — not a supported install target | manual setup; no extra ships for it | The MLX↔NumPy parity suite has passed twice on an RTX 5070 Laptop under WSL2, in 2026-07 and again 2026-09. Requires `MLX_ENABLE_TF32=0` (TF32 is on by default and silently costs precision; the PoC's recipes use the driver-level `NVIDIA_TF32_OVERRIDE=0` instead — measured equivalent, see its §"Where `MLX_ENABLE_TF32` lives"). Multipeak batches above 65,535 crashed on CUDA until [mlx#3858](https://github.com/ml-explore/mlx/issues/3858) was fixed in MLX 0.32.1; verified on that laptop on a dev build with the fix, and again at 0.32.2. No CUDA CI. Performance records for this configuration are committed under `benchmarks/records/`, and they are measurements of MLX's CUDA backend on one laptop GPU, not a supported performance claim: on that host plain NumPy beats CUDA on three of five solvers. Two of those are streaming kernels bound by PCIe under WSL2 (upstream `mlx#3861`), not by the card; the third is MLX's buffer cache, and reverses with the cache disabled. See [`docs/CUDA_BACKEND_POC.md`](docs/CUDA_BACKEND_POC.md). |
...
The [`examples/`](examples/) directory contains eight self-contained,
...
calibrating the energy axis on a metal Fermi edge, mapping how well
...
and a minimal front end — click a pixel of a fitted map to see its
fit — as the pattern for putting a viewer or GUI of your own on the
engine. They run in CI on every commit.
...
want the other 2,314 tests to mean what they say; do not use it to
